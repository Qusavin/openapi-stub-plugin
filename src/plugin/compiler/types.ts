		...
		? obj
				.map((value: ObjectValue) => {
					...
					if ('spread' in value) {
						...
						let initializer: ts.Expression | undefined = isTsNode(
							value.value,
						)
							? value.value
							: Array.isArray(value.value)
								...
								: toExpression({
										identifiers: identifiers.includes(
											value.key,
										)
		...
		: Object.entries(obj)
				.map(([key, value]) => {
					...
					let initializer: ts.Expression | undefined = toExpression({
						identifiers: identifiers.includes(key)
							? Object.keys(value)
							: [],
						shorthand,
						unescape,
